...
# Receipt ~/AI/workload-<label>-<UTC>.json, log ~/AI/workload-<label>-<UTC>.log. SOAK_MIN/COLD_TOKENS pass through.
set -uo pipefail
cd "$(dirname "$0")"
...
GLM_URL="http://127.0.0.1:$PORT/v1" GLM_LABEL="$LABEL" GLM_OUT="$OUT" GLM_MEM_HOSTS="$HOSTS" GLM_SSH_OPTS="$SSH_OPTS" \
  SOAK_MIN="${SOAK_MIN:-30}" COLD_TOKENS="${COLD_TOKENS:-280000}" LONGGEN_TOKENS="${LONGGEN_TOKENS:-12288}" CONC_LEVELS="${CONC_LEVELS:-4,8}" GLM_PHASES="${2:-warmup,sanity,coding,longgen,cold,cancel,soak,sanity_end}" \
